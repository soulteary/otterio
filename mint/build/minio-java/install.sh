...
(cd "$test_run_dir/minio-java.git"; git checkout --quiet "tags/${OTTERIO_JAVA_VERSION}")
$WGET --output-document="$test_run_dir/minio-${OTTERIO_JAVA_VERSION}-all.jar" "https://repo1.maven.org/maven2/io/minio/minio/${OTTERIO_JAVA_VERSION}/minio-${OTTERIO_JAVA_VERSION}-all.jar"
...
javac -cp "$test_run_dir/minio-${OTTERIO_JAVA_VERSION}-all.jar:$test_run_dir/spotbugs-annotations-${SPOTBUGS_VERSION}.jar:$test_run_dir/junit-${JUNIT_VERSION}.jar" "${test_run_dir}/minio-java.git/functional"/*.java
