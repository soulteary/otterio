...
// TestRunMain takes arguments from APP_ARGS env variable and calls otterio.Main(args)
// 1. Build and RUN test executable:
// $ go test -tags testrunmain -covermode count -coverpkg="./..." -c -tags testrunmain
// $ APP_ARGS="server /tmp/test" ./otterio.test -test.run "^TestRunMain$" -test.coverprofile coverage.cov
//
...
//
// $ docker run -e MINT_MODE=full -e SERVER_ENDPOINT=192.168.47.11:9000 -e ACCESS_KEY=otterioadmin -e SECRET_KEY=otterioadmin -v /tmp/mint/log:/mint/log otterio/mint
//
...
func TestRunMain(t *testing.T) {
	...
	go func() {
		// start otterio server with params from env variable APP_ARGS
		args := os.Getenv("APP_ARGS")
		if args == "" {
			log.Printf("No environment variable APP_ARGS found. Starting otterio without parameters ...")
		} else {
			log.Printf("Starting \"otterio %v\" ...", args)
		}
		otterio.Main(strings.Split("otterio.test "+args, " "))
	}()
	sig := <-cancelChan
