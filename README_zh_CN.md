...
> 评估并回填。
>
> OtterIO 维护者会持续整理这份清单,详见
> [`docs/security/upstream-cve-backlog.md`](./docs/security/upstream-cve-backlog.md)。
> 其中已经修复的部分包括 SSE 元数据注入(与 GHSA-3rh2-v3gr-35p9 等价)、
> RELEASE.2025-10-15 引入的 service account sub-policy 越权问题、`AddUser`
> PolicyName 提权(CVE-2021-43858),以及 2022–2024 年间多个 LDAP DN 规范化
> 上游公告;但仍有若干历史公告处于开放状态。
>
...
>
> **在面向不可信用户或不可信网络的生产环境中，目前不建议部署 OtterIO。**
> 漏洞披露流程与受支持版本矩阵请参考 [`SECURITY.md`](./SECURITY.md)。
