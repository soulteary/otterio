...
func (er erasureObjects) checkUploadIDExists(ctx context.Context, bucket, object, uploadID string) (err error) {
	...
	metaArr, errs := readAllFileInfo(ctx, disks, otterioMetaMultipartBucket, er.getUploadIDDir(bucket, object, uploadID), "", false)
...
func (er erasureObjects) removeObjectPart(bucket, object, uploadID, dataDir string, partNumber int) {
	...
	for index, disk := range storageDisks {
		...
		g.Go(func() error {
			...
			_ = storageDisks[index].Delete(context.TODO(), otterioMetaMultipartBucket, curpartPath, false)
			return nil
		}, index)
	}
	g.Wait()
...
func (er erasureObjects) renameAll(ctx context.Context, bucket, prefix string) {
	...
	for _, disk := range er.getDisks() {
		...
		go func(disk StorageAPI) {
			defer wg.Done()
			disk.RenameFile(ctx, bucket, prefix, otterioMetaTmpBucket, mustGetUUID())
		}(disk)
	}
	wg.Wait()
}
...
func (er erasureObjects) cleanupStaleUploadsOnDisk(ctx context.Context, disk StorageAPI, expiry time.Duration) {
	...
	readDirFn(pathJoin(diskPath, otterioMetaMultipartBucket), func(shaDir string, _ os.FileMode) error {
		return readDirFn(pathJoin(diskPath, otterioMetaMultipartBucket, shaDir), func(uploadIDDir string, _ os.FileMode) error {
			uploadIDPath := pathJoin(shaDir, uploadIDDir)
			fi, err := disk.ReadVersion(ctx, otterioMetaMultipartBucket, uploadIDPath, "", false)
			if err != nil {
				return nil
			}
			wait := er.deletedCleanupSleeper.Timer(ctx)
			if now.Sub(fi.ModTime) > expiry {
				er.renameAll(ctx, otterioMetaMultipartBucket, uploadIDPath)
			}
			wait()
			return nil
		})
	...
	readDirFn(pathJoin(diskPath, otterioMetaTmpBucket), func(tmpDir string, _ os.FileMode) error {
		if tmpDir == ".trash/" { // do not remove .trash/ here, it has its own routines
			return nil
		}
		vi, err := disk.StatVol(ctx, pathJoin(otterioMetaTmpBucket, tmpDir))
		if err != nil {
			return nil
		}
		wait := er.deletedCleanupSleeper.Timer(ctx)
		if now.Sub(vi.Created) > expiry {
			er.deleteAll(ctx, otterioMetaTmpBucket, tmpDir)
		}
		wait()
		return nil
	})
...
func (er erasureObjects) ListMultipartUploads(ctx context.Context, bucket, object, keyMarker, uploadIDMarker, delimiter string, maxUploads int) (result ListMultipartsInfo, err error) {
	...
	for _, disk = range er.getLoadBalancedDisks(true) {
		uploadIDs, err = disk.ListDir(ctx, otterioMetaMultipartBucket, er.getMultipartSHADir(bucket, object), -1)
		if err != nil {
	...
	for _, uploadID := range uploadIDs {
		...
		fi, err := disk.ReadVersion(ctx, otterioMetaMultipartBucket, pathJoin(er.getUploadIDDir(bucket, object, uploadID)), "", false)
		if err != nil {
			return result, toObjectErr(err, bucket, object)
		}
...
// incoming object at
// '.otterio.sys/multipart/bucket/object/uploads.json' on all the
...
func (er erasureObjects) newMultipartUpload(ctx context.Context, bucket string, object string, opts ObjectOptions) (string, error) {
	...
	if _, err := writeUniqueFileInfo(ctx, onlineDisks, otterioMetaMultipartBucket, uploadIDPath, partsMetadata, writeQuorum); err != nil {
		return "", toObjectErr(err, otterioMetaMultipartBucket, uploadIDPath)
	}
...
func (er erasureObjects) CopyObjectPart(ctx context.Context, _, _, dstBucket, dstObject, uploadID string, partID int, _ int64, _ int64, srcInfo ObjectInfo, _, dstOpts ObjectOptions) (pi PartInfo, e error) {
...
func (er erasureObjects) PutObjectPart(ctx context.Context, bucket, object, uploadID string, partID int, r *PutObjReader, _ ObjectOptions) (pi PartInfo, err error) {
	...
	// Read metadata associated with the object from all disks.
	partsMetadata, errs = readAllFileInfo(ctx, storageDisks, otterioMetaMultipartBucket,
		uploadIDPath, "", false)
	...
	// Need a unique name for the part being written in otterioMetaBucket to
	...
	defer func() {
		if online != len(onlineDisks) {
			er.deleteObject(context.Background(), otterioMetaTmpBucket, tmpPart, writeQuorum)
		}
	}()
	...
	for i, disk := range onlineDisks {
		...
		writers[i] = newBitrotWriter(disk, otterioMetaTmpBucket, tmpPartPath,
	...
	onlineDisks, err = rename(ctx, onlineDisks, otterioMetaTmpBucket, tmpPartPath, otterioMetaMultipartBucket, partPath, false, writeQuorum, nil)
	if err != nil {
		return pi, toObjectErr(err, otterioMetaMultipartBucket, partPath)
	}

	// Read metadata again because it might be updated with parallel upload of another part.
	partsMetadata, errs = readAllFileInfo(ctx, onlineDisks, otterioMetaMultipartBucket, uploadIDPath, "", false)
	reducedErr = reduceWriteQuorumErrs(ctx, errs, objectOpIgnoredErrs, writeQuorum)
	if reducedErr == errErasureWriteQuorum {
		return pi, toObjectErr(reducedErr, bucket, object)
	}
	...
	if _, err = writeUniqueFileInfo(ctx, onlineDisks, otterioMetaMultipartBucket, uploadIDPath, partsMetadata, writeQuorum); err != nil {
		return pi, toObjectErr(err, otterioMetaMultipartBucket, uploadIDPath)
	}
...
func (er erasureObjects) GetMultipartInfo(ctx context.Context, bucket, object, uploadID string, opts ObjectOptions) (MultipartInfo, error) {
	...
	// Read metadata associated with the object from all disks.
	partsMetadata, errs := readAllFileInfo(ctx, storageDisks, otterioMetaMultipartBucket, uploadIDPath, opts.VersionID, false)

	// get Quorum for this object
	readQuorum, _, err := objectQuorumFromMeta(ctx, partsMetadata, errs, er.defaultParityCount)
	if err != nil {
		return result, toObjectErr(err, otterioMetaMultipartBucket, uploadIDPath)
	}

	reducedErr := reduceWriteQuorumErrs(ctx, errs, objectOpIgnoredErrs, readQuorum)
	if reducedErr == errErasureReadQuorum {
		return result, toObjectErr(reducedErr, otterioMetaMultipartBucket, uploadIDPath)
	}
...
func (er erasureObjects) ListObjectParts(ctx context.Context, bucket, object, uploadID string, partNumberMarker, maxParts int, _ ObjectOptions) (result ListPartsInfo, err error) {
	...
	// Read metadata associated with the object from all disks.
	partsMetadata, errs := readAllFileInfo(ctx, storageDisks, otterioMetaMultipartBucket, uploadIDPath, "", false)
...
func (er erasureObjects) CompleteMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, parts []CompletePart, opts ObjectOptions) (oi ObjectInfo, err error) {
	...
	if onlineDisks, err = writeUniqueFileInfo(ctx, onlineDisks, otterioMetaMultipartBucket, uploadIDPath, partsMetadata, writeQuorum); err != nil {
		return oi, toObjectErr(err, otterioMetaMultipartBucket, uploadIDPath)
	}
...
func (er erasureObjects) AbortMultipartUpload(ctx context.Context, bucket, object, uploadID string, _ ObjectOptions) (err error) {
	...
	// Read metadata associated with the object from all disks.
	partsMetadata, errs := readAllFileInfo(ctx, er.getDisks(), otterioMetaMultipartBucket, uploadIDPath, "", false)
	...
	if err = er.deleteObject(ctx, otterioMetaMultipartBucket, uploadIDPath, writeQuorum); err != nil {
		return toObjectErr(err, bucket, object, uploadID)
	}
