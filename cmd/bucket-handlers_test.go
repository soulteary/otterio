...
<LocationConstraint xmlns="http://s3.amazonaws.com/doc/2006-03-01/"></LocationConstraint>`),
	...
	if err != nil {
		t.Fatalf("OtterIO %s: Failed to create an anonymous request.", instanceType)
	}
...
func testHeadBucketHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	if err != nil {
		t.Fatalf("OtterIO %s: Failed to create an anonymous request for bucket \"%s\": <ERROR> %v",
...
func testAPIDeleteMultipleObjectsHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	testCases := []struct {
		...
		// Currently anonymous users cannot delete multiple objects in OtterIO server
		{
			bucket:             bucketName,
			objects:            anonRequest,
			accessKey:          "",
	...
	for i, testCase := range testCases {
		...
		if rec.Code != testCase.expectedRespStatus {
			t.Errorf("Test %d: OtterIO %s: Expected the response status to be `%d`, but instead found `%d`", i+1, instanceType, testCase.expectedRespStatus, rec.Code)
		}
		...
		if testCase.expectedContent != nil && !bytes.Equal(testCase.expectedContent, actualContent) {
			...
			t.Errorf("Test %d : OtterIO %s: Object content differs from expected value.", i+1, instanceType)
		}
	}
