...
import (
	...
	ioutilx "github.com/soulteary/otterio/pkg/ioutil"
)
...
func testAPIHeadObjectHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	if err != nil {
		t.Fatalf("OtterIO %s: Failed to create an anonymous request for %s/%s: <ERROR> %v",
	...
	if err != nil {
		t.Errorf("OtterIO %s: Failed to create HTTP request for testing the response when object Layer is set to `nil`.", instanceType)
	}
...
func testAPICopyObjectPartHandlerSanity(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	if err != nil {
		...
		t.Fatalf("OtterIO %s : <ERROR>  %s", instanceType, err)
	}
...
func testAPINewMultipartHandlerParallel(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	for i := 0; i < 10; i++ {
		...
		go func() {
			...
			// Assert the response code with the expected status.
			if rec.Code != http.StatusOK {
				t.Errorf("OtterIO %s:  Expected the response status to be `%d`, but instead found `%d`", instanceType, http.StatusOK, rec.Code)
				return
			}
			// decode the response body.
			decoder := xml.NewDecoder(rec.Body)
			...
			err = decoder.Decode(multipartResponse)
			if err != nil {
				t.Errorf("OtterIO %s: Error decoding the recorded response Body", instanceType)
				return
			}
...
func testAPICompleteMultipartHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	for i, testCase := range testCases {
		...
		// Assert the response code with the expected status.
		if rec.Code != testCase.expectedRespStatus {
			t.Errorf("Case %d: OtterIO %s: Expected the response status to be `%d`, but instead found `%d`", i+1, instanceType, testCase.expectedRespStatus, rec.Code)
		}
		...
		if rec.Code == http.StatusOK {
			// Verify whether the bucket obtained object is same as the one created.
			if !bytes.Equal(testCase.expectedContent, actualContent) {
				t.Errorf("Test %d : OtterIO %s: CompleteMultipart response content differs from expected value. got %s, expecte %s", i+1, instanceType,
		...
		if actualError.Key != objectName {
			t.Errorf("OtterIO %s: error response object name (%s) differs from expected value (%s)", instanceType, actualError.Key, objectName)
		}
	}
...
func testAPIDeleteObjectHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	for i, testCase := range testCases {
		...
		// Assert the response code with the expected status.
		if rec.Code != testCase.expectedRespStatus {
			t.Fatalf("OtterIO %s: Case %d: Expected the response status to be `%d`, but instead found `%d`", instanceType, i+1, testCase.expectedRespStatus, rec.Code)
		}
		...
		// Assert the response code with the expected status.
		if recV2.Code != testCase.expectedRespStatus {
			t.Errorf("Case %d: OtterIO %s: Expected the response status to be `%d`, but instead found `%d`", i+1,
...
func testAPIPutObjectPartHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	for i, test := range testCases {
		...
		t.Run(fmt.Sprintf("OtterIO %s : Test case %d.", instanceType, i+1), func(t *testing.T) {
...
func testAPIListObjectPartsHandler(obj ObjectLayer, instanceType, bucketName string, apiRouter http.Handler,
	...
	if err != nil {
		t.Fatalf("OtterIO %s : %s.", instanceType, err)
	}
	...
	for i, test := range testCases {
		var reqV4, reqV2 *http.Request
		// Using sub-tests introduced in Go 1.7.
		t.Run(fmt.Sprintf("OtterIO %s: Test case %d failed.", instanceType, i+1), func(t *testing.T) {
			recV2 := httptest.NewRecorder()
			recV4 := httptest.NewRecorder()
