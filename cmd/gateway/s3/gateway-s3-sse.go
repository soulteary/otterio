...
/*
 ...
 Custom gateway encrypted objects are stored on backend as follows:
	 obj/.otterio/data   <= encrypted content
	 obj/.otterio/dare.meta  <= metadata

 When a multipart upload operation is in progress, the metadata set during
 NewMultipartUpload is stored in obj/.otterio/uploadID/dare.meta and each
 UploadPart operation saves additional state of the part's encrypted ETag and
 encrypted size in obj/.otterio/uploadID/part1/part.meta

 All the part metadata and temp dare.meta are cleaned up when upload completes
*/

// ListObjects lists all blobs in S3 bucket filtered by prefix
func (l *s3EncObjects) ListObjects(ctx context.Context, bucket string, prefix string, marker string, delimiter string, maxKeys int) (loi otterio.ListObjectsInfo, e error) {
...
func (l *s3EncObjects) ListObjectsV2(ctx context.Context, bucket, prefix, continuationToken, delimiter string, maxKeys int, fetchOwner bool, startAfter string) (loi otterio.ListObjectsV2Info, e error) {

	var objects []otterio.ObjectInfo
	var prefixes []string
	var isTruncated bool

	// filter out objects that contain a .otterio prefix, but is not a dare.meta metadata file.
	for {
		loi, e = l.s3Objects.ListObjectsV2(ctx, bucket, prefix, continuationToken, delimiter, 1000, fetchOwner, startAfter)
		if e != nil {
			return loi, otterio.ErrorRespToObjectError(e, bucket)
		}
		...
		for _, obj := range loi.Objects {
			...
			if strings.HasSuffix(obj.Name, gwdareMetaJSON) {
				objSlice := strings.Split(obj.Name, otterio.SlashSeparator+defaultOtterioGWPrefix)
		...
		for _, p := range loi.Prefixes {
			objName := strings.TrimSuffix(p, otterio.SlashSeparator)
	...
	loi.Objects = make([]otterio.ObjectInfo, 0)
...
func isGWObject(objName string) bool {
	isEncrypted := strings.Contains(objName, defaultOtterioGWPrefix)
	if !isEncrypted {
		return true
	}
	...
	pfxSlice := strings.Split(objName, otterio.SlashSeparator)
	var i1, i2 int
	for i := len(pfxSlice) - 1; i >= 0; i-- {
		p := pfxSlice[i]
		if p == defaultOtterioGWPrefix {
			i1 = i
		}
		if p == gwdareMetaJSON {
			i2 = i
	...
	// incomplete uploads would have a uploadID between defaultOtterioGWPrefix and gwdareMetaJSON
...
// HTTP Range requests.
func (l *s3EncObjects) GetObject(ctx context.Context, bucket string, key string, startOffset int64, length int64, writer io.Writer, etag string, opts otterio.ObjectOptions) error {
...
func (l *s3EncObjects) isGWEncrypted(ctx context.Context, bucket, object string) bool {
	_, err := l.s3Objects.GetObjectInfo(ctx, bucket, getDareMetaPath(object), otterio.ObjectOptions{})
	return err == nil
}
...
func (l *s3EncObjects) getGWMetadata(ctx context.Context, bucket, metaFileName string) (m gwMetaV1, err error) {
	oi, err1 := l.s3Objects.GetObjectInfo(ctx, bucket, metaFileName, otterio.ObjectOptions{})
	if err1 != nil {
		return m, err1
	}
	var buffer bytes.Buffer
	err = l.s3Objects.getObject(ctx, bucket, metaFileName, 0, oi.Size, &buffer, oi.ETag, otterio.ObjectOptions{})
	if err != nil {
		return m, err
	}
...
func (l *s3EncObjects) writeGWMetadata(ctx context.Context, bucket, metaFileName string, m gwMetaV1, o otterio.ObjectOptions) error {
...
func getPartMetaPath(object, uploadID string, partID int) string {
	return path.Join(object, defaultOtterioGWPrefix, uploadID, strconv.Itoa(partID), gwpartMetaJSON)
}

// deletes the custom dare metadata file saved at the backend
func (l *s3EncObjects) deleteGWMetadata(ctx context.Context, bucket, metaFileName string) (otterio.ObjectInfo, error) {
	return l.s3Objects.DeleteObject(ctx, bucket, metaFileName, otterio.ObjectOptions{})
}

func (l *s3EncObjects) getObject(ctx context.Context, bucket string, key string, startOffset int64, length int64, writer io.Writer, etag string, opts otterio.ObjectOptions) error {
	var o otterio.ObjectOptions
	if otterio.GlobalGatewaySSE.SSEC() {
		o = opts
	}
	...
	if startOffset < 0 {
		logger.LogIf(ctx, otterio.InvalidRange{})
	}
	...
	if startOffset > dmeta.Stat.Size || startOffset+length > dmeta.Stat.Size {
		logger.LogIf(ctx, otterio.InvalidRange{OffsetBegin: startOffset, OffsetEnd: length, ResourceSize: dmeta.Stat.Size})
		return otterio.InvalidRange{OffsetBegin: startOffset, OffsetEnd: length, ResourceSize: dmeta.Stat.Size}
	}
...
func (l *s3EncObjects) GetObjectNInfo(ctx context.Context, bucket, object string, rs *otterio.HTTPRangeSpec, h http.Header, lockType otterio.LockType, o otterio.ObjectOptions) (gr *otterio.GetObjectReader, err error) {
	var opts otterio.ObjectOptions
	if otterio.GlobalGatewaySSE.SSEC() {
		opts = o
	}
...
func (l *s3EncObjects) GetObjectInfo(ctx context.Context, bucket string, object string, o otterio.ObjectOptions) (objInfo otterio.ObjectInfo, err error) {
	var opts otterio.ObjectOptions
	if otterio.GlobalGatewaySSE.SSEC() {
		opts = o
	}
...
func (l *s3EncObjects) CopyObject(ctx context.Context, srcBucket string, srcObject string, dstBucket string, dstObject string, srcInfo otterio.ObjectInfo, s, d otterio.ObjectOptions) (objInfo otterio.ObjectInfo, err error) {
	...
	if cpSrcDstSame {
		...
		if s.ServerSideEncryption != nil && d.ServerSideEncryption != nil &&
			...
			if err = l.writeGWMetadata(ctx, dstBucket, getDareMetaPath(dstObject), gwMeta, otterio.ObjectOptions{}); err != nil {
				return objInfo, otterio.ErrorRespToObjectError(err)
			}
			return gwMeta.ToObjectInfo(dstBucket, dstObject), nil
		}
	}
	dstOpts := otterio.ObjectOptions{ServerSideEncryption: d.ServerSideEncryption, UserDefined: srcInfo.UserDefined}
...
// from the backend.
func (l *s3EncObjects) DeleteObject(ctx context.Context, bucket string, object string, opts otterio.ObjectOptions) (otterio.ObjectInfo, error) {
	// Get dare meta json
	if _, err := l.getGWMetadata(ctx, bucket, getDareMetaPath(object)); err != nil {
		logger.LogIf(otterio.GlobalContext, err)
...
func (l *s3EncObjects) DeleteObjects(ctx context.Context, bucket string, objects []otterio.ObjectToDelete, opts otterio.ObjectOptions) ([]otterio.DeletedObject, []error) {
...
func (l *s3EncObjects) ListMultipartUploads(ctx context.Context, bucket string, prefix string, keyMarker string, uploadIDMarker string, delimiter string, maxUploads int) (lmi otterio.ListMultipartsInfo, e error) {

	lmi, e = l.s3Objects.ListMultipartUploads(ctx, bucket, prefix, keyMarker, uploadIDMarker, delimiter, maxUploads)
	if e != nil {
		return
	}
	lmi.KeyMarker = strings.TrimSuffix(lmi.KeyMarker, getGWContentPath(otterio.SlashSeparator))
	lmi.NextKeyMarker = strings.TrimSuffix(lmi.NextKeyMarker, getGWContentPath(otterio.SlashSeparator))
	for i := range lmi.Uploads {
		lmi.Uploads[i].Object = strings.TrimSuffix(lmi.Uploads[i].Object, getGWContentPath(otterio.SlashSeparator))
	}
	return
}

// NewMultipartUpload uploads object in multiple parts
func (l *s3EncObjects) NewMultipartUpload(ctx context.Context, bucket string, object string, o otterio.ObjectOptions) (uploadID string, err error) {
	var sseOpts encrypt.ServerSide
	if o.ServerSideEncryption == nil {
		return l.s3Objects.NewMultipartUpload(ctx, bucket, object, otterio.ObjectOptions{UserDefined: o.UserDefined})
	}
	// Decide if sse options needed to be passed to backend
	if (otterio.GlobalGatewaySSE.SSEC() && o.ServerSideEncryption.Type() == encrypt.SSEC) ||
		(otterio.GlobalGatewaySSE.SSES3() && o.ServerSideEncryption.Type() == encrypt.S3) {
		sseOpts = o.ServerSideEncryption
	}

	uploadID, err = l.s3Objects.NewMultipartUpload(ctx, bucket, getGWContentPath(object), otterio.ObjectOptions{ServerSideEncryption: sseOpts})
	if err != nil {
		return
	}
	...
	err = l.writeGWMetadata(ctx, bucket, getTmpDareMetaPath(object, uploadID), gwmeta, otterio.ObjectOptions{})
	if err != nil {
		return uploadID, otterio.ErrorRespToObjectError(err)
	}
	return uploadID, nil
}

// PutObject creates a new object with the incoming data,
func (l *s3EncObjects) PutObject(ctx context.Context, bucket string, object string, data *otterio.PutObjReader, opts otterio.ObjectOptions) (objInfo otterio.ObjectInfo, err error) {
	var sseOpts encrypt.ServerSide
	// Decide if sse options needed to be passed to backend
	if opts.ServerSideEncryption != nil &&
		((otterio.GlobalGatewaySSE.SSEC() && opts.ServerSideEncryption.Type() == encrypt.SSEC) ||
			(otterio.GlobalGatewaySSE.SSES3() && opts.ServerSideEncryption.Type() == encrypt.S3) ||
	...
	if opts.ServerSideEncryption == nil {
		...
		return l.s3Objects.PutObject(ctx, bucket, object, data, otterio.ObjectOptions{UserDefined: opts.UserDefined})
	}

	oi, err := l.s3Objects.PutObject(ctx, bucket, getGWContentPath(object), data, otterio.ObjectOptions{ServerSideEncryption: sseOpts})
	if err != nil {
		return objInfo, otterio.ErrorRespToObjectError(err)
	}
	...
	gwMeta.Stat.ModTime = time.Now().UTC()
	if err = l.writeGWMetadata(ctx, bucket, getDareMetaPath(object), gwMeta, otterio.ObjectOptions{}); err != nil {
		return objInfo, otterio.ErrorRespToObjectError(err)
	}
...
func (l *s3EncObjects) PutObjectPart(ctx context.Context, bucket string, object string, uploadID string, partID int, data *otterio.PutObjReader, opts otterio.ObjectOptions) (pi otterio.PartInfo, e error) {

	if opts.ServerSideEncryption == nil {
		return l.s3Objects.PutObjectPart(ctx, bucket, object, uploadID, partID, data, opts)
	}

	var s3Opts otterio.ObjectOptions
	// for sse-s3 encryption options should not be passed to backend
	if opts.ServerSideEncryption != nil && opts.ServerSideEncryption.Type() == encrypt.SSEC && otterio.GlobalGatewaySSE.SSEC() {
		s3Opts = opts
	}

	uploadPath := getTmpGWMetaPath(object, uploadID)
	tmpDareMeta := path.Join(uploadPath, gwdareMetaJSON)
	_, err := l.s3Objects.GetObjectInfo(ctx, bucket, tmpDareMeta, otterio.ObjectOptions{})
	if err != nil {
		return pi, otterio.InvalidUploadID{UploadID: uploadID}
	}
	...
	gwMeta := newGWMetaV1()
	gwMeta.Parts = make([]otterio.ObjectPartInfo, 1)
	// Add incoming part.
	gwMeta.Parts[0] = otterio.ObjectPartInfo{
		Number: partID,
		ETag:   pi.ETag,
		Size:   pi.Size,
	}
	...
	if err = l.writeGWMetadata(ctx, bucket, getPartMetaPath(object, uploadID, partID), gwMeta, otterio.ObjectOptions{}); err != nil {
		return pi, otterio.ErrorRespToObjectError(err)
	}
	return otterio.PartInfo{
		Size:         gwMeta.Stat.Size,
		ETag:         otterio.CanonicalizeETag(gwMeta.ETag),
...
func (l *s3EncObjects) GetMultipartInfo(ctx context.Context, bucket, object, uploadID string, opts otterio.ObjectOptions) (result otterio.MultipartInfo, err error) {
...
func (l *s3EncObjects) ListObjectParts(ctx context.Context, bucket string, object string, uploadID string, partNumberMarker int, maxParts int, opts otterio.ObjectOptions) (lpi otterio.ListPartsInfo, e error) {
	// We do not store parts uploaded so far in the dare.meta. Only CompleteMultipartUpload finalizes the parts under upload prefix.Otherwise,
	// there could be situations of dare.meta getting corrupted by competing upload parts.
	dm, err := l.getGWMetadata(ctx, bucket, getTmpDareMetaPath(object, uploadID))
	if err != nil {
	...
	for i, part := range lpi.Parts {
		...
		if err != nil || len(partMeta.Parts) == 0 {
			return lpi, otterio.InvalidPart{}
		}
...
func (l *s3EncObjects) AbortMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, opts otterio.ObjectOptions) error {
	...
	for {
		...
		if err != nil {
			return otterio.InvalidUploadID{UploadID: uploadID}
		}
		for _, obj := range loi.Objects {
			if _, err := l.s3Objects.DeleteObject(ctx, bucket, obj.Name, otterio.ObjectOptions{}); err != nil {
				return otterio.ErrorRespToObjectError(err)
			}
			startAfter = obj.Name
		}
		continuationToken = loi.NextContinuationToken
...
func (l *s3EncObjects) CompleteMultipartUpload(ctx context.Context, bucket, object, uploadID string, uploadedParts []otterio.CompletePart, opts otterio.ObjectOptions) (oi otterio.ObjectInfo, e error) {
	...
	gwMeta.Parts = make([]otterio.ObjectPartInfo, len(uploadedParts))

	bkUploadedParts := make([]otterio.CompletePart, len(uploadedParts))
	...
	for i, part := range uploadedParts {
		partMeta, err := l.getGWMetadata(ctx, bucket, getPartMetaPath(object, uploadID, part.PartNumber))
		if err != nil || len(partMeta.Parts) == 0 {
			return oi, otterio.InvalidPart{}
		}
		bkUploadedParts[i] = otterio.CompletePart{PartNumber: part.PartNumber, ETag: partMeta.Parts[0].ETag}
	...
	if err = l.writeGWMetadata(ctx, bucket, getDareMetaPath(object), gwMeta, otterio.ObjectOptions{}); err != nil {
		return oi, otterio.ErrorRespToObjectError(err)
	}
...
func getTmpGWMetaPath(object, uploadID string) string {
	return path.Join(object, defaultOtterioGWPrefix, uploadID)
}
...
func getGWMetaPath(object string) string {
	return path.Join(object, defaultOtterioGWPrefix)
}
...
func getGWContentPath(object string) string {
	return path.Join(object, defaultOtterioGWPrefix, defaultGWContentFileName)
}
...
func (l *s3EncObjects) cleanupStaleUploads(ctx context.Context, expiry time.Duration) {
	...
	for _, b := range buckets {
		...
		for k := range expParts {
			l.s3Objects.DeleteObject(ctx, b.Name, k, otterio.ObjectOptions{})
		}
	}
}
...
func (l *s3EncObjects) getStalePartsForBucket(ctx context.Context, bucket string, expiry time.Duration) (expParts map[string]string) {
	...
	for {
		...
		for _, obj := range loi.Objects {
			startAfter = obj.Name
			if !strings.Contains(obj.Name, defaultOtterioGWPrefix) {
				continue
			}
...
func (l *s3EncObjects) DeleteBucket(ctx context.Context, bucket string, _ bool) error {
	...
	for k := range expParts {
		l.s3Objects.DeleteObject(ctx, bucket, k, otterio.ObjectOptions{})
	}
