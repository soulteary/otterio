...
func TestFormatErasureMigrate(t *testing.T) {
	...
	if err = os.MkdirAll(pathJoin(rootPath, otterioMetaBucket), os.FileMode(0755)); err != nil {
		t.Fatal(err)
	}
	...
	migratedVersion, err := formatGetBackendErasureVersion(pathJoin(rootPath, otterioMetaBucket, formatConfigFile))
	if err != nil {
		t.Fatal(err)
	}
