...
import (
	...
	"github.com/soulteary/otterio/pkg/lock"
	"github.com/soulteary/otterio/pkg/madmin"
	"github.com/soulteary/otterio/pkg/mimedb"
	"github.com/soulteary/otterio/pkg/mountinfo"
)
...
func initMetaVolumeFS(fsPath, fsUUID string) error {
	...
	// optimizing all other calls. Create otterio meta volume,
	// if it doesn't exist yet.
	metaBucketPath := pathJoin(fsPath, otterioMetaBucket)

	if err := os.MkdirAll(metaBucketPath, 0777); err != nil {
		return err
	}

	metaTmpPath := pathJoin(fsPath, otterioMetaTmpBucket, fsUUID)
	...
	metaMultipartPath := pathJoin(fsPath, otterioMetaMultipartBucket)
...
func (fs *FSObjects) Shutdown(ctx context.Context) error {
	...
	return fsRemoveAll(ctx, pathJoin(fs.fsPath, otterioMetaTmpBucket, fs.fsUUID))
}
...
func (fs *FSObjects) scanBucket(ctx context.Context, bucket string, cache dataUsageCache) (dataUsageCache, error) {
	...
	cache, err = scanDataFolder(ctx, fs.fsPath, cache, func(item scannerItem) (sizeSummary, error) {
		...
		fsMetaBytes, err := xioutil.ReadFile(pathJoin(fs.fsPath, otterioMetaBucket, bucketMetaPrefix, bucket, object, fs.metaJSONFile))
...
func (fs *FSObjects) DeleteBucket(ctx context.Context, bucket string, forceDelete bool) error {
	...
	if !forceDelete {
		...
		tmpBucketPath := pathJoin(fs.fsPath, otterioMetaTmpBucket, bucket+"."+mustGetUUID())
	...
	otterioMetadataBucketDir := pathJoin(fs.fsPath, otterioMetaBucket, bucketMetaPrefix, bucket)
	if err = fsRemoveAll(ctx, otterioMetadataBucketDir); err != nil {
		return toObjectErr(err, bucket)
	}
...
func (fs *FSObjects) CopyObject(ctx context.Context, srcBucket, srcObject, dstBucket, dstObject string, srcInfo ObjectInfo, srcOpts, dstOpts ObjectOptions) (oi ObjectInfo, err error) {
	...
	if cpSrcDstSame && srcInfo.metadataOnly {
		fsMetaPath := pathJoin(fs.fsPath, otterioMetaBucket, bucketMetaPrefix, srcBucket, srcObject, fs.metaJSONFile)
...
func (fs *FSObjects) GetObjectNInfo(ctx context.Context, bucket, object string, rs *HTTPRangeSpec, h http.Header, lockType LockType, opts ObjectOptions) (gr *GetObjectReader, err error) {
	...
	rwPoolUnlocker := func() {}
	if bucket != otterioMetaBucket && lockType != noLock {
		fsMetaPath := pathJoin(fs.fsPath, otterioMetaBucket, bucketMetaPrefix, bucket, object, fs.metaJSONFile)
...
func (fs *FSObjects) putObject(ctx context.Context, bucket string, object string, r *PutObjReader, opts ObjectOptions) (objInfo ObjectInfo, retErr error) {
	...
	fsTmpObjPath := pathJoin(fs.fsPath, otterioMetaTmpBucket, fs.fsUUID, tempObj)
...
func (fs *FSObjects) DeleteObject(ctx context.Context, bucket, object string, opts ObjectOptions) (objInfo ObjectInfo, err error) {
	...
	otterioMetaBucketDir := pathJoin(fs.fsPath, otterioMetaBucket)
	fsMetaPath := pathJoin(otterioMetaBucketDir, bucketMetaPrefix, bucket, object, fs.metaJSONFile)
	if bucket != otterioMetaBucket {
	...
	if bucket != otterioMetaBucket {
		...
		err = fsDeleteFile(ctx, otterioMetaBucketDir, fsMetaPath)
		if err != nil && err != errFileNotFound {
			return objInfo, toObjectErr(err, bucket, object)
		}
	}
