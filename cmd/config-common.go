...
func readConfig(ctx context.Context, objAPI ObjectLayer, configFile string) ([]byte, error) {
	...
	r, err := objAPI.GetObjectNInfo(ctx, otterioMetaBucket, configFile, nil, http.Header{}, readLock, ObjectOptions{})
	if err != nil {
...
func deleteConfig(ctx context.Context, objAPI objectDeleter, configFile string) error {
	_, err := objAPI.DeleteObject(ctx, otterioMetaBucket, configFile, ObjectOptions{})
...
func saveConfig(ctx context.Context, objAPI ObjectLayer, configFile string, data []byte) error {
	...
	_, err = objAPI.PutObject(ctx, otterioMetaBucket, configFile, NewPutObjReader(hashReader), ObjectOptions{MaxParity: true})
	return err
}

func checkConfig(ctx context.Context, objAPI ObjectLayer, configFile string) error {
	if _, err := objAPI.GetObjectInfo(ctx, otterioMetaBucket, configFile, ObjectOptions{}); err != nil {
		// Treat object not found as config not found.
		if isErrObjectNotFound(err) {
			return errConfigNotFound
		}
