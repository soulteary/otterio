# OtterIO Logging Quickstart Guide
This document explains how to configure OtterIO server to log to different logging targets.

## Log Targets
OtterIO supports currently two target types
...
```
mc admin config set myotterio logger_webhook:name1 auth_token="" endpoint="http://endpoint:port/path"
...
OtterIO also honors environment variable for HTTP target logging as shown below, this setting will override the endpoint settings in the OtterIO server config.
```
export OTTERIO_LOGGER_WEBHOOK_ENABLE_target1="on"
export OTTERIO_LOGGER_WEBHOOK_AUTH_TOKEN_target1="token"
export OTTERIO_LOGGER_WEBHOOK_ENDPOINT_target1=http://localhost:8080/otterio/logs
...
```
mc admin config set myotterio audit_webhook:name1 auth_token="" endpoint="http://endpoint:port/path"
mc admin service restart myotterio
```

NOTE: `http://endpoint:port/path` is a placeholder value to indicate the URL format, please change this accordingly as per your configuration.

OtterIO also honors environment variable for HTTP target Audit logging as shown below, this setting will override the endpoint settings in the OtterIO server config.
```
export OTTERIO_AUDIT_WEBHOOK_ENABLE_target1="on"
export OTTERIO_AUDIT_WEBHOOK_AUTH_TOKEN_target1="token"
export OTTERIO_AUDIT_WEBHOOK_ENDPOINT_target1=http://localhost:8080/otterio/logs
export OTTERIO_AUDIT_WEBHOOK_CLIENT_CERT="/tmp/cert.pem"
export OTTERIO_AUDIT_WEBHOOK_CLIENT_KEY=="/tmp/key.pem"
otterio server /mnt/data
```
...
```json
{
  ...
  "userAgent": "OtterIO (linux; amd64) otterio-go/v6.0.32 mc/2019-08-12T18:27:13Z",
  "requestHeader": {
    "Authorization": "AWS4-HMAC-SHA256 Credential=otterio/20190812/us-east-1/s3/aws4_request,SignedHeaders=host;x-amz-content-sha256;x-amz-date;x-amz-decoded-content-length,Signature=d3f02a6aeddeb29b06e1773b6a8422112890981269f2463a26f307b60423177c",
    "Content-Length": "686",
    "Content-Type": "application/octet-stream",
    "User-Agent": "OtterIO (linux; amd64) otterio-go/v6.0.32 mc/2019-08-12T18:27:13Z",
  ...
  "responseHeader": {
    ...
    "Server": "OtterIO/DEVELOPMENT.2019-08-12T21-28-07Z",
    "Vary": "Origin",
