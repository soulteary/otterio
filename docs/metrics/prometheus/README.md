# How to monitor OtterIO server with Prometheus
...
OtterIO exports Prometheus compatible data by default as an authorized endpoint at `/otterio/v2/metrics/cluster`. Users looking to monitor their OtterIO instances can point Prometheus configuration to scrape data from this endpoint. This document explains how to setup Prometheus and configure it to scrape data from OtterIO servers.
...
OtterIO supports two authentication modes for Prometheus either `jwt` or `public`, by default OtterIO runs in `jwt` mode. To allow public access without authentication for prometheus metrics set environment as follows.

```
export OTTERIO_PROMETHEUS_AUTH_TYPE="public"
otterio server ~/test
```
...
```yaml
scrape_configs:
- job_name: otterio-job
  bearer_token: <secret>
  metrics_path: /otterio/v2/metrics/cluster
  scheme: http
  static_configs:
...
If Prometheus endpoint authentication type is set to `public`. Following prometheus config is sufficient to start scraping metrics data from OtterIO.
...
scrape_configs:
- job_name: otterio-job
  metrics_path: /otterio/v2/metrics/node
  scheme: http
  static_configs:
  - targets: ['localhost:9000']
```
...
Here `prometheus.yml` is the name of configuration file. You can now see OtterIO metrics in Prometheus dashboard. By default Prometheus dashboard is accessible at `http://localhost:9090`.
