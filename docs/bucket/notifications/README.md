...
Various event types supported by OtterIO server are
...
Use client tools like `mc` to set and listen for event notifications using the [`event` sub-command](https://docs.min.io/docs/minio-client-complete-guide#events). OtterIO SDK's [`BucketNotification` APIs](https://docs.min.io/docs/golang-client-api-reference#SetBucketNotification) can also be used. The notification message OtterIO sends to publish an event is a JSON message with the following [structure](https://docs.aws.amazon.com/AmazonS3/latest/dev/notification-content-structure.html).
...
- Install and configure OtterIO Server from [here](https://docs.min.io/docs/minio-quickstart-guide).
- Install and configure OtterIO Client from [here](https://docs.min.io/docs/minio-client-quickstart-guide).
...
> - When configured using environment variables, the `:name` can be specified using this format `OTTERIO_NOTIFY_WEBHOOK_ENABLE_<name>`.
...
OtterIO requires a 5.x series version of Elasticsearch. This is the latest major release series. Elasticsearch provides version upgrade migration guidelines [here](https://www.elastic.co/guide/en/elasticsearch/reference/current/setup-upgrade.html).
...
Use `mc admin config set` command to update the configuration for the deployment. Restart the OtterIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:otterio:sqs::1:elasticsearch` at start-up if there were no errors.

```sh
$ mc admin config set myotterio notify_elasticsearch:1 queue_limit="0"  url="http://127.0.0.1:9200" format="namespace" index="otterio_events" queue_dir="" username="" password=""
```
...
With the `mc` tool, the configuration is very simple to add. Let us say that the OtterIO server is aliased as `myotterio` in our mc configuration. Execute the following:
...
Use curl to view contents of `otterio_events` index.
...
The OtterIO server configuration file is stored on the backend in json format.The Redis configuration is located in the `redis` key under the `notify` top-level key. Create a configuration key-value pair here for your Redis instance. The key is a name for your Redis endpoint, and the value is a collection of key-value parameters described in the table below.
...
Use `mc admin config set` command to update the configuration for the deployment.Restart the OtterIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:otterio:sqs::1:redis` at start-up if there were no errors.
...
In the previous terminal, you will now see the operation that OtterIO performs on Redis:
...
Here we see that OtterIO performed `HSET` on `otterio_events` key.

In case, `access` format was used, then `otterio_events` would be a list, and the OtterIO server would have performed an `RPUSH` to append to the list. A consumer of this list would ideally use `BLPOP` to remove list items from the left-end of the list.
...
Install [PostgreSQL](https://www.postgresql.org/) database server. For illustrative purposes, we have set the "postgres" user password as `password` and created a database called `otterio_events` to store the events.
...
Use `mc admin config set` command to update the configuration for the deployment. Restart the OtterIO server to put the changes into effect. The server will print a line like `SQS ARNs: arn:otterio:sqs::1:postgresql` at start-up if there were no errors.
...
Install MySQL from [here](https://dev.mysql.com/downloads/mysql/). For illustrative purposes, we have set the root password as `password` and created a database called `otteriodb` to store the events.
...
Open MySQL terminal and list the rows in the `otterio_images` table.
...
Use `mc admin config set` command to update the configuration for the deployment. Here the endpoint is the server listening for webhook notifications. Save the settings and restart the OtterIO server for changes to take effect. Note that the endpoint needs to be live and reachable when you restart your OtterIO server.
...
We used [Thumbnailer](https://github.com/minio/thumbnailer) to listen for OtterIO notifications when a new JPEG file is uploaded (HTTP PUT). Triggered by a notification, Thumbnailer uploads a thumbnail of new image to OtterIO server. To start with, download and install Thumbnailer.
...
Then open the Thumbnailer config file at `config/webhook.json` and add the configuration for your OtterIO server and then start Thumbnailer by

```
NODE_ENV=webhook node thumbnail-webhook.js
```

Thumbnailer starts running at `http://localhost:3000/`. Next, configure the OtterIO server to send notifications to this URL (as mentioned in step 1) and use `mc` to set up bucket notifications (as mentioned in step 2). Then upload a JPEG image to OtterIO server by
