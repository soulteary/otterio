...
```bash
export OTTERIO_CACHE="on"
export OTTERIO_CACHE_DRIVES="/mnt/drive1,/mnt/drive2,/mnt/cache{1...3}"
export OTTERIO_CACHE_EXCLUDE="*.pdf,mybucket/*"
...
In the example above this means that  `OTTERIO_CACHE_WATERMARK_LOW` is effectively `0.8 * 0.7 * 100 = 56%` and the `OTTERIO_CACHE_WATERMARK_HIGH` is effectively `0.8 * 0.9 * 100 = 72%` of total disk space.
