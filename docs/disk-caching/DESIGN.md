...
```
otterio gateway <name> -h
...
...
  CACHE:
     OTTERIO_CACHE_DRIVES: List of mounted cache drives or directories delimited by ","
     OTTERIO_CACHE_EXCLUDE: List of cache exclusion patterns delimited by ","
     OTTERIO_CACHE_QUOTA: Maximum permitted usage of the cache in percentage (0-100).
     OTTERIO_CACHE_AFTER: Minimum number of access before caching an object.
     OTTERIO_CACHE_WATERMARK_LOW: % of cache quota at which cache eviction stops
     OTTERIO_CACHE_WATERMARK_HIGH: % of cache quota at which cache eviction starts
     OTTERIO_CACHE_RANGE: set to "on" or "off" caching of independent range requests per object, defaults to "on"


...
...

  Start OtterIO gateway to s3 with edge caching enabled on '/mnt/drive1', '/mnt/drive2' and '/mnt/export1 ... /mnt/export24',
     ...
     $ export OTTERIO_CACHE_DRIVES="/mnt/drive1,/mnt/drive2,/mnt/export{1..24}"
     $ export OTTERIO_CACHE_EXCLUDE="mybucket/*,*.pdf"
     $ export OTTERIO_CACHE_QUOTA=80
     $ export OTTERIO_CACHE_AFTER=3
     $ export OTTERIO_CACHE_WATERMARK_LOW=70
     $ export OTTERIO_CACHE_WATERMARK_HIGH=90

     $ otterio gateway s3
```

### Run OtterIO gateway with cache on Docker Container
### Stable
...
- The cache drives are required to be a filesystem mount point with [`atime`](http://kerolasa.github.io/filetimes.html) support to be enabled on the drive. Alternatively writable directories with atime support can be specified in OTTERIO_CACHE_DRIVES
...
- All range GET requests are cached by default independently, this may be not desirable in all situations when cache storage is limited and where downloading an entire object at once might be more optimal. To optionally turn this feature off, and allow downloading entire object in the background `export OTTERIO_CACHE_RANGE=off`.
- To ensure security guarantees, encrypted objects are normally not cached. However, if you wish to encrypt cached content on disk, you can set OTTERIO_CACHE_ENCRYPTION_MASTER_KEY environment variable to set a cache KMS
master key to automatically encrypt all cached content.

  Note that cache KMS master key is not recommended for use in production deployments. If the OtterIO server/gateway machine is ever compromised, the cache KMS master key must also be treated as compromised.
...
Upon restart of otterio gateway after a running otterio process is killed or crashes, disk caching resumes automatically. The garbage collection cycle resumes and any previously cached entries are served from cache.
