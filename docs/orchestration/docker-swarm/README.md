# Deploy OtterIO on Docker Swarm

Docker Engine provides cluster management and orchestration features in Swarm mode. OtterIO server can be easily deployed in distributed mode on Swarm to create a multi-tenant, highly-available and scalable object store.
...
## 3. Create Docker secrets for OtterIO
...
## 4. Deploy distributed OtterIO services

The example OtterIO stack uses 4 Docker volumes, which are created automatically by deploying the stack. We have to make sure that the services in the stack are always (re)started on the same node, where the service is deployed the first time.
...
Before deploying the stack, add labels to the Docker nodes where you want the otterio services to run:

```
docker node update --label-add otterio1=true <DOCKER-NODE1>
docker node update --label-add otterio2=true <DOCKER-NODE2>
docker node update --label-add otterio3=true <DOCKER-NODE3>
docker node update --label-add otterio4=true <DOCKER-NODE4>
```
...
Remove the distributed OtterIO services and related network by

```shell
docker stack rm otterio_stack
```
Swarm doesn't automatically remove host volumes created for services. This may lead to corruption when a new OtterIO service is created in the swarm. So, we recommend removing all the volumes used by OtterIO, manually. To do this, logon to each node in the swarm and run
...
## 5. Accessing OtterIO services

The services are exposed, by default, on the internal overlay network by their services names (otterio1, otterio2, ...).
The docker-compose.yml file also exposes the OtterIO services behind a single alias on the otterio_distributed network.

Services in the Swarm which are attached to that network can interact with the host "otterio-cluster" instead of individual services' hostnames.  This provides a simple way to loosely load balance across all the OtterIO services in the Swarm as well as simplifies configuration and management.
...
* OtterIO services in the Docker compose file expose ports 9001 to 9004. This allows multiple services to run on a host. Explore other configuration options in [Docker documentation](https://docs.docker.com/compose/compose-file/#/ports).
