...
OtterIO服务器允许针对特定的对象一次写入，多次读取 (WORM),或者通过使用默认的对象锁定配置设置存储桶，该配置将默认的保留模式和保留期限应用于所有对象。这将使存储桶中的对象不可改变，也就是说，直到存储桶的对象锁定配置或对象保留中指定的时间到期后，才允许删除版本。
...
- 如果不需要系统时间来设置保留日期，则可以将*OTTERIO_NTP_SERVER*环境变量设置为远程NTP server endpoint。
