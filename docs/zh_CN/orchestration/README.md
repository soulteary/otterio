...
OtterIO是一个[云原生](https://baike.baidu.com/item/Cloud%20Native/19865304?fr=aladdin)的应用程序，旨在在多租户环境中以可持续的方式进行扩展。编排（orchestration）平台为OtterIO的扩展提供了非常好的支撑。以下是各种编排平台的OtterIO部署文档:
...
容器提供了隔离的应用执行环境，编排平台通过容器管理以及复制功能提供了无缝的扩展。OtterIO继承了这些，针对每个租户提供了存储环境的隔离。

OtterIO是建立在云原生的基础上，有纠删码、分布式和共享存储这些特性。OtterIO专注于并且只专注于存储，而且做的还不错。它可以通过编排平台复制一个OtterIO实例就实现了水平扩展。
