# 使用Docker Compose部署OtterIO

Docker Compose允许定义和运行单主机，多容器Docker应用程序。

使用Compose，您可以使用Compose文件来配置OtterIO服务。 然后，使用单个命令，您可以通过你的配置创建并启动所有分布式OtterIO实例。 分布式OtterIO实例将部署在同一主机上的多个容器中。 这是建立基于分布式OtterIO的开发，测试和预发布环境的好方法。
...
## 2. 在Docker Compose上运行分布式OtterIO

在Docker Compose上部署分布式OtterIO,请下载[docker-compose.yaml](https://github.com/minio/minio/blob/master/docs/orchestration/docker-compose/docker-compose.yaml?raw=true)到你的当前工作目录。Docker Compose会pull OtterIO Docker Image,所以你不需要手动去下载OtterIO binary。然后运行下面的命令
...
* 默认情况下会创建4个otterio实例，你可以添加更多的OtterIO服务（最多总共16个）到你的OtterIO Compose deployment。添加一个服务
  * 复制服务定义并适当地更改新服务的名称。
  * 更新每个服务中的命令部分。
...
* Docker compose file中的OtterIO服务使用的端口是9001到9004，这允许多个服务在主机上运行。
...
- [使用Docker Swarm部署OtterIO](https://docs.min.io/cn/deploy-minio-on-docker-swarm)
