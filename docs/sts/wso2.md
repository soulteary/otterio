...
WSO2 is an Identity Server open source and is released under Apache Software License Version 2.0, this document covers configuring WSO2 to be used as an identity provider for OtterIO server STS API.
...
WSO2 generates tokens in first style by default, but if to be used with OtterIO we should configure WSO2 to provide JWT tokens instead.
...
Using the above `access_token` we can perform an STS request to OtterIO to get temporary credentials for OtterIO API operations. OtterIO STS API uses [JSON Web Key Set Endpoint](https://docs.wso2.com/display/IS541/JSON+Web+Key+Set+Endpoint) to validate if JWT is valid and is properly signed.
...
### 5. Setup OtterIO with OpenID configuration URL
OtterIO server expects environment variable for OpenID configuration url as `OTTERIO_IDENTITY_OPENID_CONFIG_URL`, this environment variable takes a single entry.
```
export OTTERIO_IDENTITY_OPENID_CONFIG_URL=https://localhost:9443/oauth2/oidcdiscovery/.well-known/openid-configuration
